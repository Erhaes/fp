...
const char *ssid = "Lucky_rahman";
const char *password = "gangsimbul170845";
String postData ; 
String link = "http://192.168.1.9/Biometrcis/getdata.php"; 
int FingerID = 0;     
uint8_t id;
...
const unsigned long dataCheckInterval = 5000;
...
void loop() {
  ...
  // Cek status pengiriman data setiap 5 detik
...
void SendFingerprintID(int finger) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Cannot send fingerprint data: No WiFi connection");
    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(WHITE);
    display.setCursor(0,0);
    display.print(F("No WiFi Connection"));
    display.display();
    return;
  }
  ...
  Serial.println("Sending fingerprint data...");
  ...
  Serial.println("Data transmission result:");
  ...
  if (httpCode > 0) {
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (payload.substring(0, 5) == "login") {
        ...
        Serial.print("User logged out: ");
        Serial.println(user_name);
      }
    } else {
      Serial.println("Server returned error response");
      display.clearDisplay();
      display.setTextSize(1);
      display.setTextColor(WHITE);
      display.setCursor(0,0);
      display.print(F("Server Error"));
      display.display();
    }
  } else {
    ...
    display.setTextColor(WHITE);
    display.setCursor(0,0);
    display.print(F("Failed to send data"));
    display.display();
  }
...
uint8_t deleteFingerprint(int id) {
  ...
  if (p == FINGERPRINT_OK) {
    ...
    display.display();
    return p; // Kembalikan nilai di jalur else
  }   
}
...
uint8_t getFingerprintEnroll() {
  ...
  return 0; // Tambahkan return default jika tidak ada kondisi lain
}
//******************Check if there a Fingerprint ID to add******************
void confirmAdding(){
...
void connectToWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    display.setTextColor(WHITE);
    display.setCursor(0, 0);
    display.print(F("WiFi Connection Failed"));
    display.display();
  }
}